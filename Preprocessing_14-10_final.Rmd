...
```{r}
# Configuración general de la ventana de gráficos
par(mfrow = c(3, 3),            # cuadrícula de 3x3
    mar = c(4, 4, 3, 1),        # márgenes más amplios
    oma = c(0, 0, 2, 0),        # margen exterior (para título general)
    las = 1,                    # etiquetas horizontales
    cex.axis = 0.8,             # tamaño de ejes
    cex.names = 0.8,            # tamaño de nombres en ejes
    cex.main = 0.9)             # tamaño de títulos

# Colores predefinidos (más suaves que rainbow)
  ...
  for (varc2 in varCat2) {
    if (varc1 != varc2) {
      ...
      barplot(prop_table, beside = TRUE,
              ...
              border = "white",              # bordes blancos para contraste
              ylim = c(0, max(prop_table) * 1.2),  # espacio superior
              main = paste(varc1, "vs", varc2),    # título descriptivo
              ylab = "Proporció",
              xlab = varc2,
              legend.text = TRUE,            # leyenda automática
              args.legend = list(bty = "n", cex = 0.7, x = "topright")) # sin borde
    }
  }
}

# Título general de toda la figura
mtext("Comparación de proporciones entre variables categóricas", 
